...
def generate_types_file(classes: List[CSharpClass]) -> str:
    ...
    for cls in sorted(classes, key=lambda x: x.name):
        ...
        for prop in cls.properties:
            ...
            # C#のnullable型(?付き)またはis_optionalの場合はTypeScriptでもoptionalに
            optional = "?" if (is_nullable or prop.is_optional) else ""
    ...
    for controller, eps in sorted(by_controller.items()):
        ...
        for ep in eps:
            ...
            if ep.method == "GET":
                ...
                lines.append(f"  return useMutation<{response_type}, Error, {request_type}>({{")
                lines.append(f"    mutationFn: (data) => {controller}Api.{func_name}(data),")
                lines.append("    onSuccess: (...args) => {")
                
                # Invalidate related queries
                lines.append(f"      queryClient.invalidateQueries({{ queryKey: ['{controller}'] }});")
                lines.append("      options?.onSuccess?.(...args);")
                lines.append("    },")
                lines.append("    ...options,")
                lines.append("  });")
                lines.append("}")
