...
### 優先度別タスク数
- 🔴 重要度：高 - **3件**（通知機能、Google OAuth、テストコード）※issueとしては5件（バックエンド・フロントエンド分離）
- 🟡 重要度：中 - **5件**（パスワードリセット、自動保存、棋譜再生、Atomic Design、エラーハンドリング）※issueとしては8件
- 🟢 重要度：低 - **5件**（XSS対策、CI/CD、パフォーマンス、アクセシビリティ、ドキュメント）※issueとしては7件
- **合計: 20件のissue**（詳細は `docs/issues-to-create.md` を参照）
